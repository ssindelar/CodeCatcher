...
public class CodeCatcherConnector extends AbstractExtensionConnector implements
		NativePreviewHandler {
	private static final long serialVersionUID = -2609631912936019843L;

	CodeCatcherServerRpc rpc = RpcProxy
			.create(CodeCatcherServerRpc.class, this);
	...
	protected void extend(ServerConnector target) {
		handler = com.google.gwt.user.client.Event
	 ...
	 * "[DEV] onPreviewNativeEvent: type: "+event.getNativeEvent().getType());
	 * logger.log(Level.INFO,
	 * "[DEV] onPreviewNativeEvent: char: "+event.getNativeEvent
	 * ().getCharCode()); logger.log(Level.INFO,
	 * "[DEV] onPreviewNativeEvent: key : "
	 * +event.getNativeEvent().getKeyCode()); logger.log(Level.INFO,
	 * "[DEV] AON: reset buffer"); logger.log(Level.INFO,
	 * "[DEV] AON: append: "+this.code);
	 * 
	 * logger.log(Level.INFO,
	 * "[DEV] onPreviewNativeEvent: not reading and start");
	 * logger.log(Level.INFO, "[DEV] onPreviewNativeEvent: reading and end");
	 * logger.log(Level.INFO, "[DEV] onPreviewNativeEvent: reading char"+(char)
	 * keyCode );
	 */
	...
	public void onPreviewNativeEvent(NativePreviewEvent event) {
		...
		if (event.getNativeEvent().getType().equals(Event.KEYPRESS)) {
			...
			// Alt+F4 und Ctrl+F4 unterdrücken
			if (keyCode == KeyCode.F4
					&& (event.getNativeEvent().getAltKey() || event
							.getNativeEvent().getCtrlKey())) {
				logger.log(Level.FINE, "[DEV] suppressing Alt+F4 or Ctrl+F4");
				event.cancel();
			}
	...
	private boolean checkForShortcut(NativePreviewEvent event) {
		...
		if (!nativeEvent.getShiftKey() && charCode >= 97 && charCode <= 122) {
			// logger.log(Level.INFO,
			// "[DEV] checkForShortcut: shift not pressed");
			charCode -= 32;
		}
	...
	private void sendBufferToServer(boolean isScanCode) {
		CodeCatchedEvent serverEvent = new CodeCatchedEvent();
		serverEvent.setCode(this.code);
		serverEvent.setScanCodeFlag(isScanCode);
	...
	public void onStateChanged(StateChangeEvent stateChangeEvent) {
		...
		// logger.log(Level.INFO,
		// "[DEV] codeStartKeyCode: "+this.codeStartKeyCode);
		// logger.log(Level.INFO, "[DEV] codeEndKeyCode: "+this.codeEndKeyCode);
		// logger.log(Level.INFO,
		// "[DEV] isSkipReturn: "+this.isSkipFollowingReturn);
